...
//            RuntimeInit.wtf(tag, what, system);
     ...
     */
//    @FastNative
    public static boolean isLoggable(@Nullable String tag, @Level int level) { return true; }
    /**
     * Send a {@link #WARN} log message and log the exception.
     * @param tag Used to identify the source of a log message.  It usually identifies
     *        the class or activity where the log call occurs.
...
//    @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
    ...
    public static int printlns(int bufID, int priority, @Nullable String tag, @NotNull String msg,
        ...
        PrintWriter lbbw = new PrintWriter(new BufferedWriter(logWriter, bufferSize));
        lbbw.println(msg);
        if (tr != null) {
            // This is to reduce the amount of log spew that apps do in the non-error
            // condition of the network being unavailable.
